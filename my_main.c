     ...
     for (int frame = start_frame; frame <= end_frame; frame++){
       ...
       if (curr == NULL) {
        ...
        // head node exists
         ...
         // while next node exist, node not found
         while (curr->next && strcmp(curr->name,name) != 0) {
           curr = curr->next;
         }
         // if node found, modify node
...
void my_main() {
  ...
  for (f = 0; f < num_frames; f++) {
      ...
      vn = knobs[f];
      while (vn) {
        sym = lookup_symbol(vn->name);
        set_value(sym, vn->value);
        vn = vn->next;
      }
